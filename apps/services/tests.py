...
class ServiceCategoryModelTest(TestCase):
    def setUp(self):
        self.category = ServiceCategory.objects.create(
            name="Консультации", slug="consultations", description="Врачебные консультации"
        )
...
class ServiceModelTest(TestCase):
    def setUp(self):
        self.category = ServiceCategory.objects.create(name="Анализы", slug="analyses")
...
class ServiceCategorySerializerTest(TestCase):
    def setUp(self):
        self.category = ServiceCategory.objects.create(name="Диагностика", slug="diagnostics")
...
class ServiceSerializerTest(APITestCase):
    def setUp(self):
        self.category = ServiceCategory.objects.create(name="УЗИ", slug="ultrasound")
        self.service = Service.objects.create(
            category=self.category,
...
class ServiceDetailViewTest(TestCase):
    def setUp(self):
        self.client = Client()
        self.category = ServiceCategory.objects.create(name="Хирургия", slug="surgery")
        self.service = Service.objects.create(
            category=self.category,
...
class ServiceViewSetTest(TestCase):
    def setUp(self):
        ...
        self.category = ServiceCategory.objects.create(name="Кардиология", slug="cardiology")
        self.service = Service.objects.create(
            category=self.category,
            name="ЭКГ",
            slug="ecg",
...
class ServiceIntegrationTest(TestCase):
    def setUp(self):
        self.client = Client()
        self.category = ServiceCategory.objects.create(name="Стоматология", slug="dentistry")
        self.service1 = Service.objects.create(
            category=self.category,
            name="Чистка зубов",
            slug="teeth-cleaning",
