...
class DoctorProfileModelTest(TestCase):
    ...
    def setUpTestData(cls):
        cls.user = User.objects.create_user(
            username="doctor1", email="doctor@test.com", password="testpass123", is_doctor=True
        )
        cls.profile = DoctorProfile.objects.create(user=cls.user, specialty="Cardiology", bio="Test bio")
...
class UserSignalsTest(TestCase):
    def test_doctor_profile_creation(self):
        user = User.objects.create_user(username="dr_test", email="dr@test.com", password="testpass", is_doctor=True)
